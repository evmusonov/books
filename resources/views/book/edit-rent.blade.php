...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="form">
                <h1>Редактирование книги в аренду <a href="/user/{{ Auth::user()->login }}/books" class="float-right h6 mt-3">Назад</a></h1>
                @if (session('exist'))
                ...
                <form method="POST" action="/books/{{ $book->id }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <div class="row mb-3">
                            <div class="col">
                                ...
                                <input type="text" name="rent_amount" class="form-control" value="{{ $book->rent_amount }}">
                            </div>
                            <div class="col">
                                <label>Период</label>
                                <select class="custom-select" name="rent_type_id">
                                    @foreach(Book::rentPeriod() as $index => $period)
                                        <option value="{{ $index }}" {{ $book->rent_type_id == $index ? 'selected' : '' }}>{{ $period }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col">
                    ...
                    <button type="submit" class="btn btn-primary custom-button mt-3">Сохранить</button>
            </form>
        </div>
    </div>
@endsection
